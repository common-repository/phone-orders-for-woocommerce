		...
		<div v-else>
			...
			<label>
				...
				<span class="wpo-gifts-list__product__item__checkbox">
				    <input type="checkbox" v-model="selectedProducts" :value="product.id" :disabled="countGiftsLeft <= 0 && selectedProducts.indexOf(product.id) === -1">
				</span>
				...
				<div class="wc-order-item-missing-variation-attribute" v-for="(variation_attribute, index) in product.missing_variation_attributes" :key="variation_attribute.key">
				    <strong>
					...
					<option v-for="valueLabel in variation_attribute.values" :value="valueLabel.value">{{valueLabel.label}}</option>
				    </select>
				</div>
			    </span>
			</label>
            ...
            <template v-slot:footer>
                <div style="width: 100%">
                    <b-row>
                        ...
                        <b-button @click="addToCart" variant="primary" :disabled="!!!selectedProducts.length">{{ addToCartLabel }}</b-button>
