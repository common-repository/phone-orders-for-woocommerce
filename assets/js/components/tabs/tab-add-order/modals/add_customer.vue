				...
				v-show="!!group.fields.length"
                            ...
                            <b-row>
                                <b-col cols="12" md="6" v-for="field in group.fields" :key="field.key">
                                    <span>
                                        <label class="mr-sm-2" :for="field.key" :class="[ emptyRequiredFieldsList.indexOf(field.key) > -1 ? 'text-danger' : '' ]">{{ requiredFieldsList.indexOf(field.key) > -1 ? field.label + ' *' : field.label }}</label>
